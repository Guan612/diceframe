...
def test_copy_edit_export_import_and_delete_custom_adventure(tmp_path: Path) -> None:
    api = _api(tmp_path)
    copied = adventures.copy_adventure(api, "core:lanterns_of_greymoor", {
    ...
    detail = adventures.adventure_detail(api, "user:my_greymoor", "zh-CN")["adventure"]
    ...
    updated = adventures.update_adventure(
        api, "user:my_greymoor", {"files": files}, "zh-CN",
    )
    ...
    filename, payload = adventures.export_adventure(api, "user:my_greymoor")
    assert filename == "my_greymoor.dfadventure.zip"
    imported_api = _api(tmp_path / "imported")
    imported = adventures.import_adventure(imported_api, payload, "imported_greymoor")
    ...
    assert adventures.delete_adventure(api, "user:my_greymoor") == {
...
def test_create_adventure_starts_with_a_valid_editable_package(tmp_path: Path) -> None:
    api = _api(tmp_path)
    created = adventures.create_adventure(api, {
    ...
    detail = adventures.adventure_detail(api, "user:fog_harbor_case", "zh-CN")["adventure"]
...
def test_builtin_and_bound_adventures_are_protected(tmp_path: Path) -> None:
    ...
    builtin = adventures.adventure_detail(
        api, "core:lanterns_of_greymoor", "zh-CN",
    )["adventure"]
    with pytest.raises(PermissionError, match="built-in"):
        adventures.update_adventure(
            api, "core:lanterns_of_greymoor", {"files": builtin["files"]}, "zh-CN",
        )
    with pytest.raises(PermissionError, match="built-in"):
        adventures.delete_adventure(api, "core:lanterns_of_greymoor")

    adventures.copy_adventure(api, "core:lanterns_of_greymoor", {
    ...
    detail = adventures.adventure_detail(api, "user:bound_story", "zh-CN")["adventure"]
    ...
    with pytest.raises(PermissionError, match="bound to a save"):
        adventures.update_adventure(
            api, "user:bound_story", {"files": detail["files"]}, "zh-CN",
        )
    with pytest.raises(PermissionError, match="bound to a save"):
        adventures.delete_adventure(api, "user:bound_story")
...
def test_import_rejects_core_identity_and_archive_traversal(tmp_path: Path) -> None:
    ...
    _filename, builtin_payload = adventures.export_adventure(
        api, "core:lanterns_of_greymoor",
    )
    with pytest.raises(ValueError, match="core namespace"):
        adventures.import_adventure(api, builtin_payload, "forged_core")
