...
# NapCat 重连退避：从 reconnect_delay_sec（默认 5s）递增 ×2，封顶 60s，连上后复位。
# 避免连不上时每 5 秒刷屏。
...
class NapCatTransport:
    ...
    async def run(self) -> None:
        ...
        backoff = max(1.0, float(self.config.reconnect_delay_sec or 1))
        while not self._stop:
            try:
                ...
                self.logger.warning("NapCat 连接失败，%.0f 秒后重试: %s", backoff, self.config.ws_url)
            finally:
                self._ws = None
                self._fail_pending("NapCat 连接已断开")
            if not self._stop:
                await asyncio.sleep(backoff)
                backoff = min(backoff * _RECONNECT_BACKOFF_FACTOR, _RECONNECT_MAX_SEC)
