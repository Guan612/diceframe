...
def gm_language_instruction(value: object) -> str:
    ...
    if lang == "en":
        return (
            ...
            "- Never narrate a fee, purchase, or reward as completed unless the "
            "same response emits the corresponding PAY, TEAM_PAY, or GOLD "
            "proposal tag; an omitted tag means no currency change occurred."
        )
    if lang == "ja":
        return (
            "## 出力言語\n"
            ...
            "プレイヤー向けの散文だけ翻訳する。\n"
            "- 同じ応答で PAY、TEAM_PAY、または GOLD の提案タグを出していない限り、"
            "料金・購入・報酬を完了したものとして描写しないこと。タグがなければ通貨は変化しない。"
        )
    return (
        "## 输出语言\n"
        ...
        "- 除非同一响应输出对应的 PAY、TEAM_PAY 或 GOLD 提案标签，否则不得把费用、购买或奖励叙述为已完成；"
        "没有标签就不会发生金币变化。"
    )
