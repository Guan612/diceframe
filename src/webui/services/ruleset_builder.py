...
def _context(api: "WebAPI", rule_id: str, language: str, draft: Any):
...
def experience(api: "WebAPI", rule_id: str, language: str = "") -> dict[str, Any]:
    rule = api._load_rule_by_id(rule_id, language)
    if rule is None:
...
def choices(
    api: "WebAPI", rule_id: str, draft: Any, language: str = "",
) -> dict[str, Any]:
    rule, runtime, draft, error = _context(api, rule_id, language, draft)
    if error:
        return error
