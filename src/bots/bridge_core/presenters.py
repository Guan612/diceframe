...
def recap_text(detail: dict[str, Any], language: str = "zh-CN") -> str:
    ...
    lines = (
        ["Recap:", f"  Current: round {round_no}, scene “{scene}”."]
        if english else
        ["前情提要：", f"　　当前：第 {round_no} 轮，场景「{scene}」。"]
    )
    narrative = str(recap.get("narrative") or "").strip()
    if narrative:
        lines.append(f"  Overview: {narrative}" if english else f"　　总览：{narrative}")
    recent = recap.get("recent_rounds") if isinstance(recap.get("recent_rounds"), list) else []
    if recent:
        lines.append("Recent events:" if english else "最近发生：")
        for item in recent[-3:]:
            ...
            for action in actions[:3]:
                if isinstance(action, dict):
                    ...
                    if text:
                        action_bits.append(f"{name}: {text}" if english else f"{name}：{text}")
            body = gm_text or ("; ".join(action_bits) if english else "；".join(action_bits))
            if body:
                lines.append(f"  R{item.get('round', '?')}: {body}" if english else f"　　R{item.get('round', '?')}：{body}")
    ...
    if len(lines) <= 2:
        lines.append(
            "  No previous rounds yet; submit an action to begin."
            if english else
            "　　暂无历史回合；可以先发送行动开始冒险。"
        )
    return "\n".join(lines)
...
def map_lines(data: dict[str, Any], language: str = "zh-CN") -> list[str]:
    ...
    if not locations:
        base = [
            "No map data yet. Add locations to the lorebook or continue the story."
            if english else
            "暂无地图数据；可以先在世界书补地点，或继续推进剧情。"
        ]
        if current_scene:
            base.insert(0, f"Current scene: {current_scene}" if english else f"当前场景：{current_scene}")
        return base
    ...
    lines = [
        f"Current scene: {current_scene or 'Unknown'}"
        if english else
        f"当前场景：{current_scene or '未知'}"
    ]
    ...
    for loc in locations:
        ...
        if len(edges) >= 8:
            break
    if edges:
        lines.append(("Connections: " + "; ".join(edges)) if english else ("连接：" + "；".join(edges)))
    if len(locations) > 10:
        lines.append(
...
def map_text(lines: list[str], language: str = "zh-CN") -> str:
    title = "Scene map:\n" if bridge_is_english(language) else "场景地图：\n"
    indent = "  " if bridge_is_english(language) else "　　"
...
def payment_line(payment: dict[str, Any], index: int, language: str = "zh-CN") -> str:
    ...
    return (
        f"{index}. R{round_no} {amount} gold: {reason}"
        if english else
        f"{index}. R{round_no} {amount} 金币：{reason}"
    )
...
def character_creation_lines(
    ...
    *,
    ...
    attr_line = (
        "Attributes: " + (", ".join(attr_names[:8]) if attr_names else "follow the web form")
        if english else
        "属性：" + ("、".join(attr_names[:8]) if attr_names else "按网页表单填写")
    )
    if attr_total:
        attr_line += f" (suggested total: {attr_total})" if english else f"（建议总点数 {attr_total}）"
    skill_bits = []
    if max_skills:
        skill_bits.append(f"choose about {max_skills}" if english else f"建议选 {max_skills} 个左右")
    if skill_points:
        skill_bits.append(f"suggested skill points: {skill_points}" if english else f"参考技能点 {skill_points}")
    if max_skill_value:
        skill_bits.append(f"suggested per-skill maximum: {max_skill_value}" if english else f"单项参考 {max_skill_value}")
    if skill_examples:
        skill_bits.append(("Examples: " + ", ".join(skill_examples[:6])) if english else ("例：" + "、".join(skill_examples[:6])))
    if skill_hint:
...
def character_creation_text(lines: list[str], link: str = "", language: str = "zh-CN") -> str:
    english = bridge_is_english(language)
    text = ("Create a character:\n" if english else "新建角色 / 车卡：\n") + "\n".join(lines)
    if link:
        text += f"\nWeb character creator: {link}" if english else f"\n网页建卡入口：{link}"
    return text


def character_draft_lines(draft: dict[str, Any], language: str = "zh-CN") -> list[str]:
    english = bridge_is_english(language)
    name = str(draft.get("character_name") or ("Unnamed character" if english else "未命名角色"))
    race = str(draft.get("race") or ("Unspecified identity" if english else "未定身份"))
    cls = str(draft.get("class") or ("Unspecified role" if english else "未定定位"))
    ...
    if english:
        ...
        if equipment:
            lines.append(f"Equipment/items: {equipment}")
        return lines
    lines = [
        f"角色：{name}",
        f"身份/定位：{race} · {cls}",
...
def background_lines(background: str, language: str = "zh-CN") -> list[str]:
    ...
    if not raw:
        return ["Background: none yet" if bridge_is_english(language) else "背景：暂无背景"]
    segments = [re.sub(r"[ \t]+", " ", seg.strip()) for seg in re.split(r"\n\s*\n", raw) if seg.strip()]
    if not segments:
        segments = [re.sub(r"\s+", " ", raw)]
    lines: list[str] = ["Background:" if bridge_is_english(language) else "背景："]
...
def character_draft_text(title: str, lines: list[str], link: str = "", language: str = "zh-CN") -> str:
    english = bridge_is_english(language)
    text = title + (":\n" if english else "：\n") + "\n".join(
        "" if line == "" else ("  " if english else "　　") + line for line in lines
    )
    if link:
        text += f"\nWeb character creator: {link}" if english else f"\n网页建卡入口：{link}"
    return text
...
def format_character_attrs(attrs: Any, language: str = "zh-CN") -> str:
    if not isinstance(attrs, dict) or not attrs:
        return "Follow the web rules" if bridge_is_english(language) else "按网页规则填写"
    separator = ", " if bridge_is_english(language) else "、"
...
def format_character_skills(skills: Any, language: str = "zh-CN") -> str:
    if not isinstance(skills, list) or not skills:
        return "Choose based on the character’s role" if bridge_is_english(language) else "按角色定位选择"
    names: list[str] = []
    for item in skills[:8]:
        if isinstance(item, dict):
            ...
            if value:
                names.append(value)
    separator = ", " if bridge_is_english(language) else "、"
    return separator.join(names) or ("Choose based on the character’s role" if bridge_is_english(language) else "按角色定位选择")
...
def player_tutorial_text(lines: list[str], language: str = "zh-CN") -> str:
    title = "New player quick start:\n" if bridge_is_english(language) else "群聊跑团新玩家一图流：\n"
    return title + "\n".join(lines)
