...
test('two clients establish a direct data channel through Hub signaling', async ({ browser, request }) => {
  ...
  await host.getByTestId('peer-direct-consent').locator('input').check()
  await host.getByRole('button', { name: '创建临时直连房间' }).click()
  // 批量开房会同时生成已绑定角色与空闲席位的链接；该流程需要空闲席位来创建新角色。
  const invitePanel = host.getByTestId('peer-invite')
  ...
  await guest.getByTestId('peer-direct-consent').locator('input').check()
  await guest.getByRole('button', { name: '连接房主' }).click()

  await expect(host.getByTestId('peer-status').locator('header [data-peer-state="connected"]')).toHaveText('P2P 直连成功', {
    timeout: 20_000,
  })
  await expect(guest.getByTestId('peer-status').locator('header [data-peer-state="connected"]')).toHaveText('P2P 直连成功', {
    timeout: 20_000,
  })
  await expect(host.locator('[data-testid="peer-invite-peer-state"][data-peer-state="connected"]')).toHaveCount(1)
  // 通道状态块进入 active 即代表心跳自检通过；不锁具体文案，避免文案调整破坏 e2e。
  await expect(host.locator('[data-testid="peer-connection-check"][data-active="true"]')).toBeVisible()
  await expect(guest.locator('[data-testid="peer-connection-check"][data-active="true"]')).toBeVisible()
...
test('a separate peer client joins a DND 2024 table with a canonical professional character', async ({ browser, request }, testInfo) => {
  ...
  await guest.getByTestId('peer-direct-consent').locator('input').check()
  await guest.getByRole('button', { name: '连接房主' }).click()
  await expect(guest.locator('[data-peer-state="connected"]')).toContainText('P2P 直连成功', { timeout: 20_000 })

  await guest.getByRole('button', { name: '进入冒险' }).click()
  const builder = guest.locator('.ruleset-experience--dnd2024')
  await expect(builder.getByRole('heading', { name: '创建你的冒险者' })).toBeVisible()
  await builder.getByTestId('preset-card').first().click()
  ...
  const characterCenter = guest.getByTestId('professional-character-center')
