...
const description = computed(() => {
  ...
  if (reward.value) return t('economyRewardContent', {
...
<template>
  <article class="economy-proposal-card" :class="`economy-proposal-${tone}`" aria-live="polite">
    ...
    <p class="economy-proposal-help"><NIcon :component="WarningOutline" size="14" />{{ help }}</p>
    <footer class="economy-proposal-actions">
      ...
      <button type="button" class="primary" :disabled="busy" @click="emit('confirm')">
        {{ reward ? t('economyApproveReward') : t('confirmPurchase') }}
      </button>
    </footer>
  </article>
</template>
